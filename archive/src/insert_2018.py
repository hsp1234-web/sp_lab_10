...
def insert_2018():
    ...
    log_path = Path(r"c:\SP_lab_Projects\sp_lab_v9_1.1\data\insert_2018_log.txt")
    ...
    with open(log_path, 'w', encoding='utf-8') as log:
        try:
            ...
            count_before = con.execute("SELECT COUNT(*) FROM futures_data WHERE Date LIKE '2018%'").fetchone()[0]
            log.write(f"Rows for 2018 before insert: {count_before}\n")
            
            if count_before > 0:
                log.write("2018 data already exists? Then why verify showed 0?\n")
            ...
            log.write("Insert command executed.\n")
            
            # Verify
            count_after = con.execute("SELECT COUNT(*) FROM futures_data WHERE Date LIKE '2018%'").fetchone()[0]
            log.write(f"Rows for 2018 after insert: {count_after}\n")
