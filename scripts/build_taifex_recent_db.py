...
將最近30天的官方資料整合到taifex_recent_data.db中：
- Put/Call比資料
- 選擇權Delta值
- 期貨每日行情
- 選擇權每日行情
...
class TaifexRecentDBBuilder:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.db_path = self.project_root / "data" / "taifex_recent_data.db"
    ...
    def _init_database(self):
        ...
        try:
            ...
            # Put/Call比資料表 (調整為整體統計)
            conn.execute("""
                CREATE TABLE IF NOT EXISTS pcr_data (
                    date VARCHAR,
                    symbol VARCHAR DEFAULT 'TXO',
                    put_volume DOUBLE,
                    call_volume DOUBLE,
                    pcr_ratio DOUBLE,
                    total_volume DOUBLE,
        ...
        try:
            ...
            for csv_file in pcr_files:
                try:
                    ...
                    # 建立正確順序的DataFrame
                    required_cols = ['date', 'symbol', 'put_volume', 'call_volume', 'pcr_ratio',
                                   'total_volume', 'put_open_interest', 'call_open_interest', 'pcr_open_interest']

                    # 建立新的DataFrame，確保欄位順序正確
                    df_ordered = pd.DataFrame()
                    for col in required_cols:
                        if col in df.columns:
                            df_ordered[col] = df[col]
                        else:
                            df_ordered[col] = None  # 對於不存在的欄位設為None

                    df = df_ordered

                    if not df.empty:
                        # 調試：檢查DataFrame的欄位順序和前幾筆資料
                        self.logger.info(f"DataFrame欄位順序: {df.columns.tolist()}")
                        self.logger.info(f"前筆資料: {df.iloc[0].to_dict()}")
    ...
    def build_options_delta_database(self):
        ...
        try:
            ...
            self.logger.warning("注意：台灣期貨交易所目前未提供Delta值資料的直接下載API")
            self.logger.info("如需Delta資料，建議使用其他資料來源或自行計算")
            ...
            if not delta_files:
                self.logger.info("未找到選擇權Delta值資料檔案，跳過處理")
                return
...
def main():
    ...
    try:
        # 初始化建置器
        builder = TaifexRecentDBBuilder(logger)
